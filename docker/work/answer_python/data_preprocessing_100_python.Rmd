...
print(pkg_resources.get_distribution('geopy').version)
...
df_p019 = df_p019.loc[:, ['customer_id', 'amount', 'rank']].head(n=10)
...
df_p020 = df_p020.loc[:, ['customer_id', 'amount', 'rank']].head(n=10)
...
sales_ymdはstrなので日時に変換する。group_by()すると列がインデックスになるので、インデックスを無くすには(Rでいうdplyr::ungroupするには)reset_indexする()。
...
公式では agg({'sales_ymd':['max','min']}) を使っている。確かにその方が分かりやすい。
...
最頻値を直接得る方法はないので、頻度を調べて最も高い項目を選ぶ。公式解答とは解き方が異なる。

★ できれば項目だけでなく、何回出現したかも知りたい。

```{python p029_1_print, eval=TRUE, echo=TRUE, results='asis', cache=FALSE}
df_p029_1 = df_receipt.loc[:, ['store_cd', 'product_cd']].groupby('store_cd').agg(lambda x:x.value_counts().index[0])
print(df_p029_1.to_markdown())
```

```{python p029_2_print, eval=TRUE, echo=TRUE, results='asis', cache=FALSE}
df_p029_2 = df_receipt.loc[:, ['store_cd', 'product_cd']].groupby('store_cd').agg(pd.Series.mode)
print(df_p029_2.to_markdown())
```
...
df_p032 = df_receipt.loc[:, ['amount']].quantile([.0, .25, .5, .75, 1])
...
DatetimeIndexを使って年月日を取り出す。公式解答では pd.to_datetime.dt を用いている。
...
出力される列がRと微妙に異なると思ったら、これまでの問題の出力は使わず、df_receiptを読み直すのだった。

```{python p074_print, eval=TRUE, echo=TRUE, results='asis', cache=FALSE}
df_p074 = df_receipt.loc[:, ['customer_id', 'sales_ymd']]
df_p074['sales_ymd'] = pd.to_datetime(df_p074['sales_ymd'], format='%Y%m%d')
df_p074['dayofweek'] = pd.DatetimeIndex(df_p074['sales_ymd']).dayofweek
df_p074['week_start'] = [df_p074.loc[i, 'sales_ymd'] + relativedelta(days=-int(df_p074.loc[i, 'dayofweek'])) for i in range(df_p074.shape[0])]
print(df_p074.loc[:,['customer_id', 'week_start', 'sales_ymd', 'dayofweek']].head(n=10).to_markdown())
```
...
公式解答では train_test_split を用いている。
...
print(df_customer[df_customer.gender=="不明"].shape)
print(df_p076[df_p076.gender=="男性"].shape)
print(df_p076[df_p076.gender=="女性"].shape)
print(df_p076[df_p076.gender=="不明"].shape)
```
...
公式解答に従い、 ascending を複数渡す (ascendingにboolのリストを渡せることは、Pandasのマニュアルに書いてある)。30行減ったのはRと同じである。
...
df_customer_u = df_customer.merge(df_p087, how='left').fillna(value={'amount':0}).sort_values(['amount', 'customer_id'], ascending=[False, True]).drop_duplicates(subset=['customer_name', 'postal_cd'])
print(df_customer.shape[0] - df_customer_u.shape[0])
```
