...
df_p012 = df_store[df_store['address'].apply(lambda x: x.find('横浜市') >= 0)].head(n=10)
...
df_p024 = df_receipt.loc[:, ['customer_id', 'sales_ymd']].groupby('customer_id').apply(
    lambda x: x.sort_values(by='sales_ymd', ascending=False)).head(10)
...
df_p025 = df_receipt.loc[:, ['customer_id', 'sales_ymd']].groupby('customer_id').apply(
    lambda x: x.sort_values(by='sales_ymd', ascending=True)).head(10)
...
df_p029 = df_receipt.loc[:, ['store_cd', 'amount']].groupby('store_cd').agg(lambda x:x.value_counts().index[0])
...
df_p036 = df_receipt.merge(df_store, how='inner')[col_names].head(10)
print(df_p036.to_markdown())
```
...
df_receiptと結合するとNaNだらけでよく分からないので、diffだけ取ったものを先に示す。

```{python p042_print, eval=TRUE, echo=TRUE, results='asis', cache=FALSE}
df_p042_diff1 = df_receipt.loc[:, ['amount', 'sales_ymd']].groupby('sales_ymd').sum().diff(periods=1).reset_index().rename(columns={'amount': 'diff1'})
df_p042_diff2 = df_receipt.loc[:, ['amount', 'sales_ymd']].groupby('sales_ymd').sum().diff(periods=2).reset_index().rename(columns={'amount': 'diff2'})
df_p042_diff3 = df_receipt.loc[:, ['amount', 'sales_ymd']].groupby('sales_ymd').sum().diff(periods=3).reset_index().rename(columns={'amount': 'diff3'})
df_p042_diffs = df_p042_diff1.merge(df_p042_diff2, on='sales_ymd', how='left').merge(df_p042_diff3, on='sales_ymd', how='left').head(n=10)
print(df_p042_diffs.to_markdown())
df_p042 = df_p042_diffs.merge(df_receipt, on='sales_ymd', how='inner').head(n=10)
print(df_p042.to_markdown())
```
...
df_sales_summary = pd.melt(df_sales_summary, id_vars=['age'], var_name='gender', value_name='amount')
...
df_p053 = df_p053.loc[:, ['tokyo', 'customer_id']].groupby('tokyo').nunique()
...
df_p056['age'] = ['{}s'.format(x) for x in np.minimum(((df_customer['age'] // 10) * 10).to_list(), 60)]
...
df_p057['age'] = ['{}s'.format(x) for x in np.minimum(((df_customer['age'] // 10) * 10).to_list(), 60)]
...
df_p063['profit'] = (df_p063['unit_price'] - df_p063['unit_cost']) / df_p063['unit_price']
...
df_p064['profit'] = (df_p064['unit_price'] - df_p064['unit_cost']) / df_p064['unit_price']
print(df_p064.head(n=10).to_markdown())
```
...
print(df_r070_date.loc[:,['sales_ymd', 'dayofweek', 'week_start']].head(n=10).to_markdown())
```
